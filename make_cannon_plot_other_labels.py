...
def get_plot(sample,n,file):
	teststars=np.loadtxt(dd+'{}/{}.txt'.format(sample,file),usecols=[0],dtype='str')
...
label='lum'
da='cannon_vs_LLR/other_labels/'
file='testing_cannon_{}'.format(label)
get_plot(da+label,1,file)
# get_plot('dwarfs_only',2)
plt.tight_layout()
plt.subplots_adjust(wspace=0)
# plt.show(True)
plt.savefig(dd+'{}.png'.format(label),dpi=100,bbox_inches='tight')
