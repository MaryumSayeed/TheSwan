...
filename=d+'Cannon_train_stars.txt'
a = open(filename, 'r')  #fitsfile names

numtrain = 3000
al = a.readlines()[0:numtrain]

#xgrid = np.arange(3, 270,0.009) 
...
#input flux array
#correlation coefficient at each lag - same dimension as input array
...
for each in fluxall[0:numtrain]: # used to use the full 10,000 but now only use the first 2000 
    #ifluxall.append(np.fft.ifft(each)) 
    ifluxall.append(each)

#tc_fluxa_log = np.log(abs(np.real(ifluxall)))[:,0:780] #  for logg, numax and deltanu labels
tc_fluxa_log = np.log10(abs(np.asarray(ifluxall)))[:,0:21000] # for teff label
...
#plt.plot(freq[0:8500],tc_flux[1])
#plt.xlim([7.7,12.5])
#plt.show()
#exit()
...
for k in range(0,len(logg_train)):
    ...
    metaall[k,1] = teff_train[k]
    #metaall[k,2] = kp_train[k]
    #metaall[k,2] = kp_train[k]

#print(dataall,len(dataall))
#print(metaall,len(metaall))
#with open(d+'training_realifft_unweighted.pkl', "rb") as f:
#    w = pickle.load(f)

#pickle.dump(w, open(d+'training_realifft_unweighted_py2.pickle',"wb"), protocol=2)
file_in = open(d+'training_cannon.pickle', 'wb') 
